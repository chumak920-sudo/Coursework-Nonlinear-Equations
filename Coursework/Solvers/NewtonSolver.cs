...
public class NewtonSolver : INonlinearSolver
{
    public double[] Solve(SystemModel model, double[] initialGuess, double precision, int maxIterations, out int iterationsTaken, out string errorMessage)
    {
        ...
        for (int iter = 0; iter < maxIterations; iter++)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                for (int j = 0; j < n; j++)
                {
                    J[i, j] = model.CalculateDerivative(i, j, x); // Беремо точну аналітичну похідну
                }
            }
    ...
    private double[] SolveGauss(double[,] A, double[] B)
    {
        ...
        // Прямий хід Гаусса (з вибором головного елемента для стабільності)
        for (int i = 0; i < n; i++)
        {
            ...
            if (maxEl < 1e-10) return null; // Якщо головний елемент ~0, матриця вироджена
